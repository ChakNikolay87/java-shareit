...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingDto createBooking(Long userId, BookingDto bookingDto) throws ItemNotAvailableException {
        User booker = userRepository.findById(userId)
                .orElseThrow(() -> new NoSuchElementException("Пользователь с id=%d не найден".formatted(userId)));

        Item item = itemRepository.findById(bookingDto.getItemId())
                .orElseThrow(() -> new NoSuchElementException("Вещь с id=%d не найдена".formatted(bookingDto.getItemId())));

        if (!item.getAvailable()) {
            throw new ItemNotAvailableException("Вещь с id=%d недоступна для бронирования".formatted(item.getId()));
        }
    ...
    @Override
    @Transactional
    public BookingDto approveBooking(Long ownerId, Long bookingId, String approved) throws BookingAccessException {
        if (!(approved.equalsIgnoreCase("true") || approved.equalsIgnoreCase("false"))) {
            throw new IllegalArgumentException("Параметр approved должен быть 'true' или 'false'");
        }

        Booking booking = bookingRepository.findById(bookingId)
                .orElseThrow(() -> new NoSuchElementException("Бронирование с id=%d не найдено".formatted(bookingId)));

        if (!booking.getItem().getOwner().getId().equals(ownerId)) {
            throw new BookingAccessException("Только владелец вещи может одобрить или отклонить бронирование");
        }

        if (booking.getStatus() == APPROVED) {
            throw new IllegalStateException("Бронирование уже было одобрено");
        }

        if (booking.getStatus() != WAITING) {
            throw new IllegalStateException("Бронирование не может быть изменено, если оно не в ожидании");
        }
