...
public class BookingServiceImplTest {
    ...
    @Test
    public void approveBookingThrowsBookingAccessExceptionWhenUserIsNotOwner() throws ItemNotAvailableException {
        BookingDto bookingDto = new BookingDto();
        bookingDto.setItemId(item.getId());
        bookingDto.setStart(LocalDateTime.now().plusDays(1));
        bookingDto.setEnd(LocalDateTime.now().plusDays(2));
        ...
        assertThrows(BookingAccessException.class, () -> {
            bookingService.approveBooking(booker.getId(), createdBooking.getId(), "true"); // Pass a valid value like "true"
        });
    }
    ...
    public void approveBookingThrowsIllegalStateExceptionWhenBookingAlreadyApproved() throws ItemNotAvailableException, BookingAccessException {
        ...
        // Approve the booking for the first time
        bookingService.approveBooking(owner.getId(), createdBooking.getId(), "true");

        // Try approving the booking again, expecting an IllegalStateException to be thrown
    ...
    public void approveBookingThrowsIllegalStateExceptionWhenBookingAlreadyRejected() throws BookingAccessException, ItemNotAvailableException {
        ...
        bookingService.approveBooking(owner.getId(), createdBooking.getId(), "false"); // Set rejection explicitly

        assertThrows(IllegalStateException.class, () -> {
            bookingService.approveBooking(owner.getId(), createdBooking.getId(), "true"); // Now pass a valid value for approval
        });
    }
