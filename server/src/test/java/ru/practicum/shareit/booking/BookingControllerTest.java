...
public class BookingControllerTest {
    ...
    public void approveBookingTest() throws Exception {
        ...
        when(bookingService.approveBooking(any(), any(), any(String.class))).thenReturn(bookingDto);

        mockMvc.perform(patch("/bookings/1")
                        .header(X_SHARER_USER_ID, 1L)
                        .param("approved", "true")  // Параметр остается строкой
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(bookingDto)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(1L));

        verify(bookingService, times(1)).approveBooking(any(), any(), any(String.class));
    }
